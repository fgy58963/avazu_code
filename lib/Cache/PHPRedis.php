<?php
class Cache_PHPRedis
{
    protected static $CACHE = array();

    // public static function SELECTDB($db, $clusterId = 'default')
    // {
    //     $redis = Db_Redis::getInstance($clusterId);
    //     $redis->select($db);
    //     return $redis;
    // }
    ...
    public static function GET($key, $clusterId = 'default', $enableMethodCache = TRUE)
    {
        if ($enableMethodCache && isset(self::$CACHE['GET'][$key])) {
            ...
            log_message('Cache_PHPRedis GET from method cache. key:' . $key . ' value:' . json_encode($value), LOG_DEBUG);
            return $value;
        }
        $redis = Db_Redis::getInstance($clusterId);
        $value = $redis->get($key);
        ...
        log_message('Cache_PHPRedis GET. key:' . $key . ' value:' . $value, LOG_DEBUG);
        return $value;
    }
    ...
    public static function SET($key, $value, $clusterId = 'default')
    {
        ...
        log_message('Cache_PHPRedis SET. key:' . $key . ' value:' . $value . ' result:' . $result, LOG_DEBUG);
        return $result;
    }
    ...
    public static function SETEX($key, $value, $cacheTime, $clusterId = 'default')
    {
        ...
        log_message('Cache_PHPRedis SETEX. key:' . $key . ' value:' . $value . ' ttl:' . $cacheTime . ' result:' . $result, LOG_DEBUG);
        return $result;
    }
     ...
     * @return Bool TRUE if the command is successful.
     */
    public static function SETNX($key, $value, $cacheTime = 0, $clusterId = 'default')
    {
        $redis = Db_Redis::getInstance($clusterId);
        $result = $redis->setnx($key, $value);
        if ($result && $cacheTime) {
            self::EXPIRE($key, $cacheTime);
        }
        log_message('Cache_PHPRedis SETNX. key:' . $key . ' value:' . $value . ' ttl:' . $cacheTime . ' result:' . $result, LOG_DEBUG);
        return $result;
    }
    ...
    public static function EXPIRE($key, $cacheTime, $clusterId = 'default')
    {
        ...
        log_message('Cache_PHPRedis EXPIRE. key:' . $key . ' ttl:' . $cacheTime . ' result:' . $result, LOG_DEBUG);
        return $result;
    }
    ...
    public static function DEL($key, $clusterId = 'default')
    {
        ...
        log_message('Cache_PHPRedis DEL. key:' . $key . ' result:' . $result, LOG_DEBUG);
        return $result;
    }
    ...
    public static function EXISTS($key, $clusterId = 'default')
    {
        ...
        log_message('Cache_PHPRedis EXISTS. key:' . $key . ' result:' . $result, LOG_DEBUG);
        return $result;
    }
    ...
    public static function INCR($key, $clusterId = 'default')
    {
        ...
        log_message('Cache_PHPRedis INCR. key:'. $key. ' result:' . $result, LOG_DEBUG);
        return $result;
    }
    ...
    public static function INCRBY($key, $increment, $clusterId = 'default')
    {
        ...
        log_message('Cache_PHPRedis INCRBY. key:'. $key. ' decrement:' .$increment .' result:' . $result, LOG_DEBUG);
        return $result;
    }
    ...
    public static function DECR($key, $clusterId = 'default')
    {
        ...
        log_message('Cache_PHPRedis DECR. key:'. $key. ' result:' . $result, LOG_DEBUG);
        return $result;
    }
    ...
    public static function DECRBY($key, $decrement, $clusterId = 'default')
    {
        ...
        log_message('Cache_PHPRedis DECRBY. key:'. $key. ' decrement:' .$decrement .' result:' . $result, LOG_DEBUG);
        return $result;
    }
    ...
    public static function HGETALL($key, $clusterId = 'default', $enableMethodCache = TRUE)
    {
        if ($enableMethodCache && isset(self::$CACHE['HGETALL'][$key])) {
            ...
            log_message('Cache_PHPRedis HGETALL from method cache. key:' . $key . ' result:' . json_encode($result), LOG_DEBUG);
            return $result;
        }
        $redis = Db_Redis::getInstance($clusterId);
        ...
        log_message('Cache_PHPRedis HGETALL. key:' . $key . ' result:' . json_encode($result), LOG_DEBUG);
        return $result;
    }
    ...
    public static function HEXISTS($key, $field, $clusterId = 'default')
    {
        ...
        log_message('Cache_PHPRedis HEXISTS. key:' . $key . ' field:' . $field . ' result:' . $result, LOG_DEBUG);
        return $result;
    }
    ...
    public static function HMSET($key, $hashTable, $clusterId = 'default')
    {
        ...
        log_message('Cache_PHPRedis HMSET. key:' . $key . ' hashtable:' . json_encode($hashTable) . ' result:' . json_encode($result), LOG_DEBUG);
        return $result;
    }
