 ...
 * @since v1.0
 */
class Cimongo_Op extends Cimongo_Extras {
         ...
         * @since v1.0.0
         */
        public function __construct($host, $port, $user, $pass, $dbname, $query_safety = true) {
                parent::__construct($host, $port, $user, $pass, $dbname, $query_safety);
        }
        ...
        public function save($collection = "", $insert = array()) {
                ...
                $this->_inserted_id = FALSE;
                try {
                        $query = $this->db->selectCollection($collection)->save($insert, array("w" => $this->query_safety));
                        if (isset($insert['_id'])) {
                                $this->_inserted_id = $insert['_id'];
                                return TRUE;
                        } else {
         ...
         *   @since v1.0.0
         */
        public function update($collection = "", $data = array(), $options = array('upsert' => TRUE)) {
                if (empty($collection)) {
        ...
        public function update_batch($collection = "", $data = array()) {
                return $this->update($collection, $data, array('multiple' => TRUE, 'upsert' => TRUE));
        }
        ...
        public function deleteIndexs($collection) {
                $this->db->selectCollection($collection)->deleteIndexes();
        }

        public function deleteIndex($collection, $keys) {
                $this->db->selectCollection($collection)->deleteIndex($keys);
        }

        public function createColloction($collection,$options){
        ...
        public function createIndex($collection, $keys, $options= array('unique' => 1, 'dropDups' => 1)){
                return $this->db->selectCollection($collection)->createIndex($keys, $options); 
        }
}
