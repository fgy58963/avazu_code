 ...
 * @package thrift.transport
 */
class TCrypterHttpClient extends TTransport {
  ...
  protected $read_buf_ = false;

  protected $read_idx_ = 0;

  public function __construct($host, $port=80, $uri='', $scheme = 'http', $crypter_ = null) {
  ...
  function checkRead() {
    if ($this->read_buf_ === false) {
      $this->read_buf_ = @stream_get_contents($this->handle_);
      if ($this->crypter_) {
  ...
  public function read($len) {
    ...
    if ($len > ($total - $this->read_idx_))
      throw new TTransportException('TCrypterHttpClient: Could not read '.$len.' bytes', TTransportException::UNKNOWN);
  ...
  public function flush() {
    ...
    $this->read_buf_ = false;
    $this->read_idx_ = 0;

    if ($this->crypter_)
      $this->buf_ = $this->crypter_->encrypt($this->buf_);
