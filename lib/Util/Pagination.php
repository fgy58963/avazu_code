...
class Util_Pagination {
    ...
    protected $cur_link_tag = '<a class="cur page-current" href="javascript:void(0)">{page}</a>';
    ...
    protected $show_input_offset = FALSE;
    ...
    public function toHtml()
    {
        ...
        if ($this->show_input) {
            ...
            $inputUrl = $this->_getInputUrl($this->url);
            $htmlStr .= <<<EOF
跳转到第<input type="text" onkeydown="__pagedo(event, this)" class="page-text" value="{$_page}" />页
<script>
function __pagedo(eventTag, e){
    var v = e.value;
    if (v < 1) {
        v = 1;
    }
    ...
    if(currentKey==13) {
        window.location='{$inputUrl}'+ v;
    }
}
</script>
EOF;
        } elseif ($this->show_input_offset) {
            $url = preg_replace('#offset\=\d+#', 'offset=', $this->url);
            $htmlStr .= <<<EOF
跳转到第<input type="text" onkeydown="__pagedo(event, this)" class="page-text" value="{$cur_page}" style="width:40px" />页
<input type="hidden" id="__pagesize" value="$page_size" />
<script>
function __pagedo(eventTag, e){
   var event = eventTag||window.event;
   var currentKey = event.charCode||event.keyCode;
   if(currentKey==13) {
        ...
        var pagesize = parseInt($('#__pagesize').val());
        var offset = (page-1)*pagesize;
        if (offset < 0) {
            offset = 0;
        }
        window.location='{$url}'+offset;
    }
}
</script>
EOF;
    ...
    private function _getInputUrl($url)
    {
        $queryArr = array();
        if (strpos($url, '?') !== FALSE) {
            list($path, $queryStr) = explode('?', $url, 2);
        }
        ...
        $queryArr['page'] = '';
        $pageLink = implode('?', array($path, http_build_query($queryArr)));
        return $pageLink;
    }
