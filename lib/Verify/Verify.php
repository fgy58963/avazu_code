 ...
 *
	 //调用上面定义的验证码类 来生产验证码
	verify::$useNoise = true;  //是否启用噪点  
	verify::$useCurve = true;   //是否启用干扰曲线
	verify::entry(); 
...
class Verify_Verify {
    //验证码的session的下标  
    public static $seKey = 'code';   //验证码关键字
    public static $secret_key = 'Asd#4%^q#fg&+]';
    ...
    public static function entry() {
        ...
        //$_SESSION[self::$seKey] = strtolower(join('', $code)); // 把验证码保存到session, 验证时注意是大写

        $verify = md5(strtolower(join('', $code)) . self::$secret_key);
        setcookie('_vv', $verify, null, '/', 'teebik.com');
    ...
    {
        $vv = get_cookie('_vv');
        return md5(strtolower($code) . self::$secret_key) == $vv;
    }
