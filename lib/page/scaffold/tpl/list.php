...
<h2>
    <?php echo $title?>
    <?php if ($canCreate) :?>
    <a class="op" href="<?php echo v_($scaffold_config, 'create_url', "/{$scaffold_config['controller_directory']}{$scaffold_config['controller']}/create?redirect_uri=" . urlencode(get_self_full_url()))?>">添加</a>
    <?php endif;?>
</h2>
<?php
...
<table class="<?php echo v_($scaffold_config['list'], 'table_class', 'list-table')?>">
    <thead>
        ...
        <tr>
            ...
            共<strong style="color:#CC0000"><?php echo $scaffold_item_count?></strong>条记录
            </td>
        </tr>
        <?php endif;?>
        <tr class="head">
        ...
        ?>
        <th<?php echo empty($column_attrs) ? '' : ' '.$column_attrs?>>
    ...
    <tr>
        <td colspan="<?php echo count($columns) + 1?>" style="text-align:center;font-size:14px;font-weight:bold;color:#999">没有数据</td>
    </tr>
    <?php else :?>
